...
class Pancake_dashboard extends AdminController
{
    ...
    public function index()
    {
        ...
        // Mặc định: cả 2 là hôm nay (để giữ đúng hành vi cũ)
        if (empty($start_date)) $start_date = date('Y-m-d');
        if (empty($end_date))   $end_date   = date('Y-m-d');

        // ==== LẤY SỐ LIỆU (có cache 60s) ====
        $m        = $this->pancake_orders_model->get_dashboard_metrics($start_date, $end_date);
        $channels = $this->pancake_orders_model->get_channel_metrics($start_date, $end_date);
        $custSeg  = $this->pancake_orders_model->get_customer_segments_overall($start_date, $end_date);
        ...
        // ==== DATA CHO VIEW (giữ nguyên key cũ + bổ sung KH mới/cũ) ====
        $data = [];
        ...
        // KH mới/cũ theo nguồn (BỔ SUNG CHO VIEW NẾU CẦN)
